  ...
  fluidRow(
    ...
    box(width=12,title = strong("Select institutions and input cutoff p-value",style="font-size: 2.0rem;"),
      ...
      fluidRow(
        column(12,
          div(
            p("To compare multimorbidity consistency in a scatterplot, select two institutions and input cutoff p-value, then click 'Compare in the scatter plot'",
              style = "text-align: left;font-size: 2.0rem; color:black;"),
            p("If you want to update the scatterplot, you can simply re-select institutions and re-input cutoff p-value, then click 'Compare in the scatter plot'",
              style = "text-align: left;font-size: 2.0rem; color:black;")
            ),
          hr()),
        column(3,p(HTML("<b>Institution 1</b>"),span(shiny::icon("info-circle"),id = "info_ins1"),
                           selectInput(ns('xcol'), NULL,choices=vars,selected = ""),
      ...
      column(3,p(HTML("<b>Institution 2</b>"),span(shiny::icon("info-circle"),id = "info_ins2"),
                  selectInput(ns('ycol'), NULL,choices = vars,selected = ""),
      ...
      column(3,actionButton(ns("update_network"), "Compare in the scatterplot"))
    ))
    ),
    column(width=12,
    box(width=12,
        title=strong("Multimorbidity consistency with selected phecode across systems",style="font-size: 2.0rem;"),
        solidHeader=F,status="warning",
      # wellPanel(style ="margin-top: 10px;background-color: white; border-color: white;",
      conditionalPanel(
    ...
    function(input,output,session){
      ...
      selectedData <- eventReactive(input$update_network,{
        ...
        if(input$xcol!=input$ycol) {
          ...
          selected = com_sim %>%
            ...
            mutate(pthreshold=ifelse((get(p1)<input$pvalue) & (get(p2)<input$pvalue),"blue","grey")) %>%
      ...
      output$consistency_plot <- renderPlot({
        ...
        if(nrow(selectedData()[["green_selected"]])!=0){
          ...
          scale_color_manual(values=c("#6495ED","#D3D3D3"), labels=c("p-value<cut-ff p-value","p-value>=cut-ff p-value"),name="Significance") +
          guides(color = guide_legend(override.aes = list(fill = "white"))) +
          scale_fill_gradient(name = "count", trans = "log",
                              breaks = c(160000,8000,400,20,1),guide="none"
                              ) +
          theme_bw() +
          ...
          labs(caption = "Notes: Co-occurring phenotypes with the selected phenotype and a corresponding p-value < cutoff p-value will be colored green")+
